...
python build_shortest_routes.py --candidate-limit 8 --max-iterations 999
python build_shortest_routes.py --self-test
```

输出文件：

- 默认写入 `.herocraft_cache/shortest_routes.json`。
- 每个可达对象记录 `steps`、`required_ids` 和当前选中的直接 `recipe`。
- `steps` 按“需要合成的非基础对象数量”计算，同一个中间物只算一次。

参数要点：

- `--cache-dir` 指定读取详情缓存的目录，默认 `.herocraft_cache`。
- `--output` 指定输出 JSON；默认 `.herocraft_cache/shortest_routes.json`。
