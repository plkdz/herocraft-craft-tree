...
- 路途对象集合指一个四基谱表项中需要实际合成出来的非基础对象集合；基础元素不计入。下文写作 $\operatorname{PathObj}(r)$，形如：

$$
\operatorname{PathObj}(r(C)) = \{X_1, X_2, \dots, C\}
$$

- 其中 $X_i$ 是该表项中需要先合成出的非基础中间对象，$C$ 是最终目标对象；水、火、土、风不写入这个集合。
- 表项步数指路途对象集合的元素数量，即：
...
- 这里 $r(A)$ 是 $A$ 的一个四基谱表项，$r(B)$ 是 $B$ 的一个四基谱表项，$\operatorname{Combine}$ 表示把最后一步单步合成配方和两个材料表项组合成一个新的 $C$ 四基谱表项。新表项记作 $r(C)$，它的路途对象集合和步数为：
...
- 强连通分量指：依赖图中一组互相可达的对象。若材料和结果在同一个强连通分量内，这条单步合成配方可能参与循环。
- 支配指：若 $\operatorname{PathObj}(x) \subseteq \operatorname{PathObj}(y)$，则 $x$ 支配 $y$。含义是使用 $x$ 不会比使用 $y$ 需要更多对象。
- 保留槽指：`candidate-limit` 内预留给不同目的的名额，例如保留最短表项、保留不同单步合成配方、保留有共享潜力的表项。
...
- 如果存在旧表，可以先用旧表里的步数作为 $\widehat D(X)$ 做第一轮排序启发；本轮生成出新表项后，再用当前 $D(X)$ 更新后续相关单步合成配方的权重。
- 排序时不能直接使用 $\infty$，否则未知材料会永远排不到。令：

$$
H = 1 + \max(\{0\}\cup\{D(X)\mid D(X)<\infty\})
$$
...
- $H$ 表示“当前已知范围之外的一层”。未知材料仍然靠后，但不会被永久排除。
- 单步合成配方层计算配方先验权重。对 $e: A+B\to C$，材料越浅越优先：
...
- $\operatorname{Penalty}(e)$ 只惩罚搜索风险，不表示配方错误。令 $I_{\text{miss}}$、$I_{\text{cycle}}$、$I_{\text{non-desc}}$、$I_{\text{dom}}$ 分别表示：材料暂时没有四基谱表项；材料和结果位于同一个强连通分量；旧表或当前表显示某个材料不比结果更浅；旧表显示同一结果下存在路途对象集合严格更小的其他单步合成配方。它们成立时取 $1$，否则取 $0$：

$$
\operatorname{Penalty}(e)
= H(8I_{\text{miss}}+4I_{\text{cycle}}+2I_{\text{non-desc}}+I_{\text{dom}})
$$

- 成环指 $I_{\text{cycle}}=1$ 的情况；非降阶指 $I_{\text{non-desc}}=1$ 的情况；旧表支配指 $I_{\text{dom}}=1$ 的情况。
- 这个系数顺序的意思是：缺材料最靠后；成环次之；非降阶再次；旧表支配只作为最轻的延后信号。所有惩罚都只影响排序，不直接删除单步合成配方。
- 表项层计算实际生成代价。对某条单步合成配方 $e: A+B\to C$，每一对材料表项都会生成一个新的 $C$ 表项：
...
- 第一步，按路途对象集合去重。同一个 $\operatorname{PathObj}$ 只保留排序更靠前的表项；排序键为：
...
- 其中 $e_r$ 表示表项 $r$ 当前层使用的单步合成配方；$\operatorname{Tie}(r)$ 是稳定排序项，只用于保证重复运行输出顺序一致。
...
- 第三步，超过 `candidate-limit` 时才做有损截断。有损截断指可能删掉未来对上层目标有用的表项，因此不再声称安全，只是启发式预算控制。建议把 $K=\text{candidate-limit}$ 分成三类保留槽；各槽向下取整，余数归主槽：
  - 主槽：约 $3K/4$，按 $\operatorname{Steps}$、$\operatorname{RecipePrior}$、$\operatorname{MaterialCost}$ 从小到大保留。
  - 配方多样性槽：约 $K/8$，按不同单步合成配方分桶，每个桶保留该配方下最好的表项；如果桶数超过槽数，优先选择 $\operatorname{RecipePrior}$ 更小的桶。这个槽避免某一条高优先级配方垄断整个四基谱表。
  - 共享潜力槽：约 $K/8$，保留路途对象集合里包含高复用对象的表项。高复用对象可以由旧表统计得到：
...
- 其中 $r^\star(Y)$ 是旧表里对象 $Y$ 的最短表项。共享潜力分数为：
...
- 共享潜力槽按 $\operatorname{ShareScore}$ 从大到小补齐。这个槽的意义是保留一些当前对象本地不够短、但可能在上层目标里因为共享路途对象而变有价值的表项。
- 如果没有旧表，$\operatorname{Use}(X)$ 不可靠，此时共享潜力槽可以关闭；先跑一遍宽松构建，第二遍再启用旧表统计。
...
- 在这个启发式方案里，48 是总名额 $K$。筛选时会把 $K$ 分配给主槽、配方多样性槽和共享潜力槽；三个槽合起来最多仍然只保留 48 个四基谱表项。
...
- `prune_candidates()`：删除被更小路途对象集合支配的四基谱表项。
