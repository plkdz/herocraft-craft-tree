...
- `--search-candidate-limit` 控制内部传播时每个对象保留多少个四基谱表项；默认等于 `--candidate-limit`。它太小时，局部不占优但对某个最终目标高度共享的表项会在传播阶段被剪掉。
...
- 这个系数顺序的意思是：缺材料最靠后；成环次之；非降阶再次；旧表劣后度只作为最轻的延后信号。所有惩罚都只影响排序，不直接删除单步合成配方。
...
- 第三步，超过 `candidate-limit` 时才做有损截断。有损截断指可能删掉未来对上层目标有用的表项，因此不再声称安全，只是启发式预算控制。建议把 $K=\text{candidate-limit}$ 分成三类保留槽；各槽向下取整，余数归主槽：
  ...
  - 共享潜力槽：约 $K/8$，保留路途对象集合里包含高复用对象的表项。高复用对象指旧表中经常出现在其他对象最短表项里的对象。定义 $r^\star(Y)$ 为旧表里对象 $Y$ 的最短表项；定义 $\operatorname{Use}(X)$ 为对象 $X$ 的复用次数：
...
- 权重和劣后度只决定表项进入各保留槽的先后顺序，不增加总名额。只要临时表大小超过 `candidate-limit`，仍然必须截断到最多 $K$ 个四基谱表项。
...
- 当前程序没有默认启用主槽、配方多样性槽和共享潜力槽。直接把这套保留槽接进全图固定点传播后，`candidate-limit 8` 实验出现队列长期不下降的问题；因此保留槽方案暂时只作为后续排查方向，不作为默认实现。

#### 当前实现状态

- 主流程已经有 `mask_weight()`、`dependency_weights_by_bit()` 和 `weights_by_bit` 参数，但配方传播时没有把权重传入候选剪枝；输出整理阶段也没有按权重重排。
- 因此当前建树实际仍以本地 `steps` 和 `required_mask` 为主要排序依据，权重代码只保留为实验入口和自检覆盖，不会写入 `shortest_steps.json`。
- 这意味着底层表能稳定给出有界近似结果，但不能保证保留“本地排位靠后、对某个未来目标高度复用”的四基谱表项。
