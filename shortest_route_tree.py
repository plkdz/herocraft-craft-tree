...
from build_shortest_routes import SHORTEST_ROUTES_FILE, load_detail_cache
...
from shortest_route_render import build_html_document, output_path_for, render_route_tree
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--routes", default="", help=f"最少步数表路径；默认缓存目录下 {SHORTEST_ROUTES_FILE}")
...
def load_shortest_routes(path: str) -> dict[int, dict[str, Any]]:
    with open(path, "r", encoding="utf-8") as file:
        payload = json.load(file)
    routes = payload.get("routes") if isinstance(payload, dict) else None
    if not isinstance(routes, dict):
        raise RuntimeError(f"{path} 不是最少步数表。先运行 python build_shortest_routes.py")
    result: dict[int, dict[str, Any]] = {}
    for raw_id, raw_route in routes.items():
...
def main() -> None:
    ...
    try:
        ...
        route_path = str(args.routes) if args.routes else os.path.join(str(args.cache_dir), SHORTEST_ROUTES_FILE)
        routes = load_shortest_routes(route_path)
        ...
        if route is None:
            fail(f"{format_object(target, show_id=args.show_id)} 不在最少步数表里。先同步缓存并运行 python build_shortest_routes.py")
        output_format: OutputFormat = str(args.format)  # type: ignore[assignment]
        if output_format == "text":
            content = (
                f"目标：{format_object(target, show_id=args.show_id)}\n"
                f"最少步数：{route.get('steps')}\n\n"
                + "\n".join(render_route_tree(target_id, details=details, routes=routes, show_id=bool(args.show_id)))
                + "\n"
            )
        else:
            content = build_html_document(target, details=details, routes=routes, show_id=bool(args.show_id))

        output_path = str(args.output) if args.output else output_path_for(target, output_format)
        os.makedirs(os.path.dirname(output_path) or ".", exist_ok=True)
        with open(output_path, "w", encoding="utf-8") as file:
            file.write(content)
        print(f"最少步数：{route.get('steps')}")
