...
```powershell
# 第一遍：生成启发表
python shortest_steps_bottomup_build.py --candidate-limit 32 --max-iterations 99999
...
- 构建器会优先用旧表 `required_ids` 闭包规模预排序，再用 `1 + A步数 + B步数` 做同级辅助排序；`risk_score` 只作为非正常边的二级排序。
- 第一遍没有旧表启发时使用原始 FIFO 队列；第二遍读取到旧表后才启用优先队列预排序。
